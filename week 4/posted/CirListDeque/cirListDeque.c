...
Author: James Palmer
Date: 1/31/2016
Description: Implement the Deque ADT with a Circularly-Doubly-Linked List with a Sentinel.
...
int isEmptyCirListDeque(struct cirListDeque *q) {
	...
	if(q->size == 0)  //prevent overloading
		return 1;
	else
		return 0;
}
...
void reverseCirListDeque(struct cirListDeque *q)
{
	...
	do
	{
		_reverseLinks(operateLink);
		operateLink = operateLink->next;
	}
	while(operateLink != q->Sentinel);
