...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	/* */
	cirListDeque *searchStack = malloc(sizeof(cirListDeque));
	initCirListDeque(searchStack);

	addBackCirListDeque(searchStack, source);

	while (!isEmptyCirListDeque(searchStack))
	{
		Vertex* currentVert = backCirListDeque(searchStack);
		removeBackCirListDeque(searchStack);
		currentVert->isVisited = 1;
		if (currentVert == destination)
			return 1;

		for (int i = 0; i < currentVert->numNeighbors; i++)
		{
			if (!(currentVert->neighbors[i]->isVisited))
				addBackCirListDeque(searchStack, currentVert->neighbors[i]);
		}
	}
	clearVisited(g);
	removeAllCirListDeque(searchStack);
	free(searchStack);
	return 0;
}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	/* */
	cirListDeque *searchQueue = malloc(sizeof(cirListDeque));
	initCirListDeque(searchQueue);

	addBackCirListDeque(searchQueue, source);

	while (!isEmptyCirListDeque(searchQueue))
	{
		Vertex* currentVert = frontCirListDeque(searchQueue);
		removeFrontCirListDeque(searchQueue);
		currentVert->isVisited = 1;
		if (currentVert == destination)
			return 1;
		...
		{
			if (!(currentVert->neighbors[i]->isVisited))
				addBackCirListDeque(searchQueue, currentVert->neighbors[i]);
		}
	}
	clearVisited(g);
	removeAllCirListDeque(searchQueue);
	free(searchQueue);
	return 0;
}
